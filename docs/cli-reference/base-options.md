...
| [`--http-backend`](#http-backend) | Select the HTTP client backend for remote schemas. |
...
| [`--url`](#url) | Fetch a schema from a URL with custom HTTP headers. |
...
!!! tip "Usage"
    ...
    1. :material-arrow-left: `--http-backend` - the option documented here
